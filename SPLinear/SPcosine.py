...
aix=np.zeros([nc]); #Array of indexes (to order them)
aiw=np.zeros([nc]); #Array of indexes (to order them)
...
def all_func():
	b=np.zeros([nnn,nn],dtype=complex)
	alfa=np.zeros([nnn,nn],dtype=complex)
